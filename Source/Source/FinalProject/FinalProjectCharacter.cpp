...
void AFinalProjectCharacter::Tick(float DeltaTime)
{
	...
	FVector EndVector = StartFVector + FP_Gun->GetRightVector() * precision;

	DrawDebugLine(TheWorld, StartFVector, EndVector, debugColor, false, 30.0f, 1.0f, 3.0f);

	//Projectile Trajectory reflection
	//int precision = 30;

	// trajectory
		//GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, FString::Printf(TEXT("Elapsed Time: %f"), elapsed));

		Super::Tick(DeltaTime);

		FVector start = FP_Gun->GetComponentLocation();
		FVector forward = FP_Gun->GetRightVector();
		FVector previous = FP_Gun->GetComponentLocation();

		float lineDistance = 1000.0f;
		float simulationLegnth = 3;

		GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::Red, FString::Printf(TEXT("Z positon at 0 should be %f"), start.Z));

		//z position time

		for (int i = 0; i <= precision; i++)
		{
			float timeAtlteration = i * (simulationLegnth / precision);

			FVector disp = Displacement(forward * 3000, timeAtlteration, FVector(0, 0, GetWorld()->GetGravityZ()));

			FVector new_position = start + disp;

			new_position = start + disp, new_position, -1.0f, (uint8)0U, 2.0f;

			DrawDebugLine(TheWorld, previous, new_position, FColor::Magenta, false, 30.0f, 1.0f, 3.0f);
			...
			if (GetWorld()->LineTraceSingleByChannel(HitResult, previous, new_position, ECC_Visibility, TraceParams))
			{
				if (HitResult.bBlockingHit)
				{
					...
					//FVector Ray = (StartFVector + FP_Gun->GetRightVector() * precision);
					FVector Ray = (new_position - previous);
					FVector Dir = Ray - 1 * FVector::DotProduct(Normal, Ray) * Normal;

					Dir.Normalize();
					DrawDebugLine(TheWorld, HitLocation,  Dir * 1000.0f, FColor::Green, false, 30.0f, 1.0f, 3.0f);
...
void AFinalProjectCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Bind fire event
	PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &AFinalProjectCharacter::OnFire);
...
bool AFinalProjectCharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		...
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &AFinalProjectCharacter::TouchUpdate);
		return true;
	}
